  ...
  <div>
    <form>
      <fieldset>
        ...
        <!--select 엘리먼트의 위치-->
        <select class="styled-select" v-model="clan">
          <option v-for="(types, clan, index) in clans" :key="index">{{ clan }}</option>
        </select>

        <select class="styled-select" v-model="type">
          <option v-for="(species, type, index) in clans[clan]" :key="index">{{ type }}</option>
        </select>

        <select class="styled-select" v-if="clans[clan]">
          <option v-for="(animals, species, index) in clans[clan][type]" :key="index">{{ species }}</option>
        </select>
      </fieldset>
    </form>
...
/* 각 셀 사이에 구분선을 추가합니다 */
.styled-table td + td, .styled-table th + th {
...
.styled-input {
  ...
  border: 1px solid #ccc; /* 테두리와 테두리 색상 */
  border-radius: 4px;/* 모서리 둥글게 */
  box-sizing: border-box; /* 패딩과 테두리를 전체 넓이에 포함 */
  padding: 5px;/* 내부 여백 */
  font-size: 16px; /* 글꼴 크기 */
}
